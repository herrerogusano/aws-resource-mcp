    ...
    def discover(self, context: AdapterContext) -> list[Resource]:
        ...
        for region in self.regions(context):
            rest_apis = pages(
                ...
                "items",
                region=region,
                request_token="position",
                response_token="position",
            )
            for api in rest_apis:
                ...
                cache_enabled = any(
                    stage.get("cacheClusterEnabled") for stage in stages
                )
                indicators = (
                    [
                        cost_indicator(
                            "provisioned_api_cache",
                            "medium",
                            "An API Gateway cache cluster can generate hourly charges.",
                        )
                    ]
                    if context.include_cost_indicators and cache_enabled
                    else []
                )
                resources.append(
                    make_resource(
                        ...
                        details={
                            ...
                            "stages": [stage.get("stageName") for stage in stages],
    ...
    def discover(self, context: AdapterContext) -> list[Resource]:
        ...
        for region in self.regions(context):
            stacks = pages(
                ...
                "StackSummaries",
                region=region,
                parameters={"StackStatusFilter": list(self._ACTIVE_STATUSES)},
            )
            for stack in stacks:
                ...
                resources.append(
                    make_resource(
                        ...
                        identifier=stack.get("StackId") or name,
                        ...
                        details={
                            "last_updated_at": stack.get("LastUpdatedTime").isoformat()
                            if hasattr(stack.get("LastUpdatedTime"), "isoformat")
                            else stack.get("LastUpdatedTime"),
                            "deletion_time": stack.get("DeletionTime").isoformat()
                            if hasattr(stack.get("DeletionTime"), "isoformat")
                            else stack.get("DeletionTime"),
                            "resources_count": len(stack_resources),
                            "drift_status": stack.get("DriftInformation", {}).get(
                                "StackDriftStatus"
                            ),
                        }
                        if context.include_details
...
class SQSAdapter(BaseAdapter):
    metadata = AdapterMetadata(
        ...
        discovery_operations=(("sqs", "ListQueues"),),
        enrichment_operations=(("sqs", "GetQueueAttributes"),),
        paginated_operations=(("sqs", "ListQueues"),),
    )
    ...
    def discover(self, context: AdapterContext) -> list[Resource]:
        ...
        for region in self.regions(context):
            ...
            for url in urls:
                ...
                arn = (
                    f"arn:aws:sqs:{region}:{context.account_id}:{name}"
    ...
    def enrich(
        ...
        for resource in resources:
            region = resource.get("region") or context.primary_region
            name = resource.get("name")
            if not name or not context.account_id:
                enriched.append(resource)
                continue
            queue_url = (
                f"https://sqs.{region}.amazonaws.com/{context.account_id}/{name}"
            )
            attributes = context.call(
                "sqs",
                "GetQueueAttributes",
                region=region,
                QueueUrl=queue_url,
            ...
            has_dlq = (
                bool(json.loads(redrive).get("deadLetterTargetArn"))
                if redrive
                else False
            )
            indicators = (
                [
                    cost_indicator(
                        "queued_messages",
                        "low",
                        "A queue with retained messages can continue consuming request and storage capacity.",
                    )
                ]
                if context.include_cost_indicators and approximate > 0
                else []
            )
            enriched.append(
                {
                    **resource,
                    "arn": attributes.get("QueueArn") or resource.get("arn"),
                    "created_at": attributes.get("CreatedTimestamp"),
                    "details": {
                        "fifo": attributes.get("FifoQueue") == "true",
                        "approximate_messages": approximate,
                        "retention_seconds": int(
                            attributes.get("MessageRetentionPeriod", "0")
                        ),
                        "dead_letter_queue": has_dlq,
                    }
                    if context.include_details
                    else {},
                    "cost_indicators": indicators,
...
class SNSAdapter(BaseAdapter):
    metadata = AdapterMetadata(
        ...
        detail_fields=("fifo", "subscriptions_count"),
        discovery_operations=(("sns", "ListTopics"),),
        enrichment_operations=(("sns", "ListSubscriptionsByTopic"),),
            ...
            for topic in topics:
                ...
                resources.append(
                    make_resource(
                        ...
                        account_id=context.account_id,
                        state="available",
                        details={"fifo": bool(name and name.endswith(".fifo"))}
                        if context.include_details
                        else {},
                    )
                )
        ...
        for resource in resources:
            ...
            subscriptions = (
                pages(
                    ...
                    "Subscriptions",
                    region=region,
                    parameters={"TopicArn": arn},
                )
                if arn
                else []
            )
            details = dict(resource.get("details", {}))
            details["subscriptions_count"] = len(subscriptions)
            enriched.append({**resource, "details": details})
        return enriched
