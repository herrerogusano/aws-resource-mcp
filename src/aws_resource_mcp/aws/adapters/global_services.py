...
class IAMAdapter(BaseAdapter):
    metadata = AdapterMetadata(
        ...
        activity_fields=(
            ...
            ActivityField(
                "password_last_used", "functional_usage", "PasswordLastUsed", "high"
            ),
            ActivityField(
                "updated_at", "configuration_change", "PolicyUpdated", "medium"
            ),
        ),
    )
            ...
            resources.append(
                make_resource(
                    ...
                    details={
                        ...
                        "permissions_boundary": bool(user.get("PermissionsBoundary")),
                    }
        ...
        for role in pages(
            ...
            resources.append(
                make_resource(
                    ...
                    details={
                        "last_used_at": last_used.get("LastUsedDate").isoformat()
                        if hasattr(last_used.get("LastUsedDate"), "isoformat")
                        else last_used.get("LastUsedDate"),
        ...
        for policy in pages(
            ...
            resources.append(
                make_resource(
                    ...
                    region="global",
                    source="iam_api",
                    identifier=policy.get("PolicyId"),
                    arn=policy.get("Arn"),
                    name=policy.get("PolicyName"),
                    account_id=context.account_id,
                    state="attached" if policy.get("AttachmentCount") else "unattached",
                    ...
                    details={
                        ...
                        "updated_at": policy.get("UpdateDate").isoformat()
                        if hasattr(policy.get("UpdateDate"), "isoformat")
                        else policy.get("UpdateDate"),
                    }
                    if context.include_details
                    else {},
                )
            ...
            indicators = (
                [
                    cost_indicator(
                        ...
                        "medium",
                        "An enabled distribution can generate request and data-transfer charges.",
                    )
                ]
                if context.include_cost_indicators and enabled
                else []
            )
            resources.append(
                make_resource(
                    ...
                    details={
                        ...
                        "origins_count": item.get("Origins", {}).get("Quantity"),
                        "aliases_count": item.get("Aliases", {}).get("Quantity"),
                        "http_version": item.get("HttpVersion"),
                        "ipv6_enabled": item.get("IsIPV6Enabled"),
                        "last_modified": item.get("LastModifiedTime").isoformat()
                        if hasattr(item.get("LastModifiedTime"), "isoformat")
                        else item.get("LastModifiedTime"),
                    }
                    if context.include_details
                    else {},
        ...
        for zone in zones:
            ...
            indicators = (
                [
                    cost_indicator(
                        "hosted_zone",
                        "low",
                        "A hosted zone can generate recurring and query charges.",
                    )
                ]
            ...
            resources.append(
                make_resource(
                    ...
                    source="route53_api",
                    identifier=identifier,
                    arn=f"arn:aws:route53:::hostedzone/{identifier}"
                    if identifier
                    else None,
                    name=zone.get("Name"),
                    account_id=context.account_id,
