...
class ECSAdapter(BaseAdapter):
    metadata = AdapterMetadata(
        ...
        cost_indicator_types=(
            "running_fargate_task",
            "desired_service_capacity",
        ...
        for region in self.regions(context):
            ...
            for batch in self._batches(cluster_arns):
                response = context.call(
                    ...
                    region=region,
                    clusters=batch,
                    include=["ATTACHMENTS", "CONFIGURATIONS"],
                )
                for cluster in response.get("clusters", []):
                    ...
                    indicators = []
                    if context.include_cost_indicators and any(
                        provider != "FARGATE" and provider != "FARGATE_SPOT"
                        ...
                        indicators.append(
                            cost_indicator(
                                "ec2_capacity_provider",
                                "medium",
                                "An EC2 capacity provider can retain billable compute capacity.",
                            )
                        )
                    resources.append(
                        make_resource(
    ...
    def _services(
        self, context: AdapterContext, region: str, cluster_arn: str
    ) -> list[Resource]:
        arns = pages(
            context,
            "ecs",
                ...
                if context.include_cost_indicators and desired > 0:
                    indicators.append(
                        cost_indicator(
                            "desired_service_capacity",
                            "high",
                            "A service with desired tasks can keep compute capacity active.",
                        )
                    )
                resources.append(
                    make_resource(
                        ...
                        region=region,
                        source="ecs_api",
                        identifier=service.get("serviceName"),
    ...
    def _tasks(
        ...
        for batch in self._batches(arns):
            for task in context.call(
                "ecs", "DescribeTasks", region=region, cluster=cluster_arn, tasks=batch
            ).get("tasks", []):
                ...
                if (
                    ...
                    indicators.append(
                        cost_indicator(
                            "running_fargate_task",
                            "high",
                            "A running Fargate task can generate compute charges.",
                        )
                    )
