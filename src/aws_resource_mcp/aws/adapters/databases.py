    ...
    def discover(self, context: AdapterContext) -> list[Resource]:
        ...
        for region in self.regions(context):
            for item in pages(
                ...
                indicators = []
                if context.include_cost_indicators and status not in {
                    "stopped",
                    "stopping",
                }:
                    indicators.append(
                        cost_indicator(
                            "running_database",
                            "high",
                            "An available database instance can generate compute and storage charges.",
                        )
                    )
                if context.include_cost_indicators and item.get("MultiAZ"):
                    indicators.append(
                        cost_indicator(
                            "multi_az",
                            "medium",
                            "Multi-AZ deployment provisions additional database capacity.",
                        )
                    )
                if context.include_cost_indicators and item.get("Iops"):
                    indicators.append(
                        cost_indicator(
                            "provisioned_iops",
                            "medium",
                            "Provisioned database IOPS can generate additional charges.",
                        )
                    )
            ...
            for item in pages(
                ...
                if context.include_cost_indicators and status not in {
                    ...
                    indicators.append(
                        cost_indicator(
                            "running_database",
                            "high",
                            "An active database cluster can generate compute and storage charges.",
                        )
                    )
                ...
                resources.append(
                    make_resource(
                        ...
                        account_id=context.account_id,
                        state=status,
                        created_at=item.get("ClusterCreateTime"),
                        details={
                            "engine": item.get("Engine"),
                            "engine_version": item.get("EngineVersion"),
                            "engine_mode": item.get("EngineMode"),
                            "members_count": len(item.get("DBClusterMembers", [])),
                            "multi_az": item.get("MultiAZ"),
            ...
            for item in pages(
                ...
                "DBSnapshots",
                region=region,
                parameters={"SnapshotType": "manual"},
                request_token="Marker",
                response_token="Marker",
            ):
                ...
                indicators = (
                    [
                        cost_indicator(
                            "manual_snapshot",
                            "low",
                            "A retained manual snapshot consumes database backup storage.",
                        )
                    ]
                    if context.include_cost_indicators
                    else []
                )
                resources.append(
                    make_resource(
                        ...
                        source="rds_api",
                        identifier=identifier,
                        arn=item.get("DBSnapshotArn"),
                        name=identifier,
                        account_id=context.account_id,
                        state=item.get("Status"),
                        created_at=item.get("SnapshotCreateTime"),
                        details={
                            "engine": item.get("Engine"),
    ...
    def discover(self, context: AdapterContext) -> list[Resource]:
        ...
        for region in self.regions(context):
            names = pages(
                ...
                "TableNames",
                region=region,
                request_token="ExclusiveStartTableName",
                response_token="LastEvaluatedTableName",
            )
            for name in names:
                table = context.call(
                    "dynamodb", "DescribeTable", region=region, TableName=name
                ).get("Table", {})
                backup = context.call(
                    "dynamodb",
                ...
                backup_status = backup.get("PointInTimeRecoveryDescription", {}).get(
                ...
                if context.include_cost_indicators and billing_mode == "PROVISIONED":
                    indicators.append(
                        cost_indicator(
                            "provisioned_capacity",
                            "medium",
                            "Provisioned table capacity can generate charges even when idle.",
                        )
                    )
                if context.include_cost_indicators and backup_status == "ENABLED":
                    indicators.append(
                        cost_indicator(
                            "continuous_backups",
                            "low",
                            "Point-in-time recovery can generate backup charges.",
                        )
                    )
                if context.include_cost_indicators and replicas:
                    indicators.append(
                        cost_indicator(
                            "global_replication",
                            "high",
                            "Global table replicas multiply regional storage and request usage.",
                        )
                    )
                if context.include_cost_indicators and size >= 10 * 1024**3:
                    indicators.append(
                        cost_indicator(
                            "significant_storage",
                            "medium",
                            "The reported table size indicates significant storage usage.",
                        )
                    )
                resources.append(
                    make_resource(
                        ...
                        region=region,
                        source="dynamodb_api",
                        identifier=table.get("TableId") or name,
                        ...
                        details={
                            ...
                            "provisioned_throughput": {
                                "read": table.get("ProvisionedThroughput", {}).get(
                                    "ReadCapacityUnits"
                                ),
                                "write": table.get("ProvisionedThroughput", {}).get(
                                    "WriteCapacityUnits"
                                ),
                            },
                            "table_size_bytes": size,
                            "item_count": table.get("ItemCount"),
                            "encrypted": table.get("SSEDescription", {}).get("Status")
                            == "ENABLED",
                            "streams": bool(table.get("LatestStreamArn")),
                            "continuous_backups": backup_status,
