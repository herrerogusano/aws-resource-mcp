...
ADAPTER_CASES = [
        ...
        "lambda",
        {
            ("lambda", "ListFunctions"): {
                "Functions": [
                    {
                        "FunctionName": "fn",
                        "FunctionArn": "arn:aws:lambda:eu-west-1:111122223333:function:fn",
                        "MemorySize": 4096,
                    }
                ]
            }
    ...
    (
        "s3",
        {
            ("s3", "ListBuckets"): {
                "Buckets": [
                    {"Name": "bucket", "CreationDate": "2026-01-01T00:00:00+00:00"}
                ]
            },
            ("s3", "GetBucketLocation"): {"LocationConstraint": "eu-west-1"},
    ...
    (
        "dynamodb",
        {
            ("dynamodb", "ListTables"): {"TableNames": ["table"]},
    ...
    (
        "ecs",
        {
            ("ecs", "ListClusters"): {"clusterArns": ["arn:cluster"]},
    ...
    (
        "apigateway",
        {
            ("apigateway", "GetRestApis"): {"items": [{"id": "api", "name": "api"}]},
    ...
    (
        ...
        {
            ("sqs", "ListQueues"): {
                "QueueUrls": ["https://sqs.eu-west-1.amazonaws.com/111122223333/queue"]
            },
    ...
    (
        ...
        {
            ("sns", "ListTopics"): {
                "Topics": [{"TopicArn": "arn:aws:sns:eu-west-1:111122223333:topic"}]
            },
            ("sns", "ListSubscriptionsByTopic"): {"Subscriptions": [{}, {}]},
        },
        "AWS::SNS::Topic",
    ),
    (
        ...
        {
            ("iam", "ListUsers"): {
                "Users": [{"UserId": "user-id", "UserName": "user", "Arn": "arn:user"}]
            }
        },
        "AWS::IAM::User",
    ),
...
def test_every_adapter_normalizes_resources_and_serializes(
    ...
    assert resources
    resource = next(
        item for item in resources if item["resource_type"] == expected_type
    )
    ...
    assert all(
        ...
        for indicator in resource["cost_indicators"]
    )
...
def test_adapter_pagination_keeps_multiple_resources() -> None:
    responses = {
        ("lambda", "ListFunctions"): [
            {
                "Functions": [{"FunctionName": "one", "FunctionArn": "arn:one"}],
                "NextMarker": "next",
            },
            ...
            _context(
                {
                    ("lambda", "ListFunctions"): {
                        "Functions": [
                            {
                                "FunctionName": "fn",
                                "Environment": {"Variables": {"SECRET": "value"}},
                            }
                        ]
                    }
                }
